#!/bin/bash
# verify-health-check.sh — Safe local verification for WhatsApp health-check.sh.
...
run_health_check() {
  WHATSAPP_HEALTH_DATA_DIR="$DATA_DIR" \
  WHATSAPP_HEALTH_ENV_FILE="$ENV_FILE" \
  WHATSAPP_HEALTH_LAUNCHD_LOG_DIR="$LOG_DIR" \
  WHATSAPP_ALERT_DRY_RUN=1 \
  "$HEALTH_CHECK"
}
